...
The LBIM influences the ocean by: (i) applying a pressure to the ocean surface, (ii) imposing heat, salt and mass fluxes on the ocean, associated with ice melting, and (iii) affecting the upper ocean by applying a no-slip boundary condition and frictional velocity beneath the ice. Fields from the LBIM are aggregated from the Lagrangian elements to the Eulerian ocean grid before they are passed to the ocean model. 
Since LBIM applies large pressures to the ocean surface,  sudden changes to the aggregated pressure field pressure can trigger tsunamis within the ocean model, making the ocean model unrealistic.  The aggregation scheme is therefore designed such that there are no jumps in pressure as an element moves from one grid cell to the next, and total pressure is conserved as an element moves.
