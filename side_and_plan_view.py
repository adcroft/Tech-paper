...
def parseCommandLine():
	...
	parser.add_argument('-plot_cross_section', type='bool', default=True,
		                        help='''    ''')
	...
	parser.add_argument('-vmin_cross', type=float, default=0.0,
		                        help='''Minimum value used for plotting cross section''')

	parser.add_argument('-vmax_cross', type=float, default=1.0,
		                        help='''Maximum values used for plotting cross section''')
	...
	parser.add_argument('-dir_slice_num_cross', type=int, default=1,
		                        help='''The index of the transect used for cross section (in the direction not plotted''')
...
def main(args):
	...
	#What to plot?
        ...
        plot_cross_section=args.plot_cross_section
...
#
	...
	################################  Plotting melt comparison  ##########################################################
		...
		#This is here to make the axes the same for top and side
		ylim_min=args.xmin
		ylim_max=args.xmax
		...
		for n in range(1):
			...
			datamap=plot_data_field(data1,x,y,vmin,vmax,flipped,colorbar=False,cmap=cmap,title=title,xlabel='x (km)',ylabel='y (km)',ylim_min=ylim_min,\
					ylim_max=ylim_max,colorbar_units=args.colorbar_units,grounding_line=grounding_line, return_handle=True)  
			...
			#if n>-1:
			#	ax.set_yticks([])
			#if (n==0) and ( (args.field=='spread_area') or (args.field=='temp')) and (simulation=='Collapse'):
			plt.plot(np.array([130., 460.]), np.array([xvec[dashed_num], xvec[dashed_num]]),'--', color='k',linewidth=3 )
			#plt.plot(xvec,grounding_line, linewidth=3.0,color='black')
		#Creating colorbar
		fig.subplots_adjust(right=0.85)
		cbar_ax = fig.add_axes([0.88,0.55 , 0.025, 0.33])
	...
	if plot_cross_section is True:
		#fig=plt.figure(figsize=(10,10),facecolor='grey')
		#ax = fig.add_subplot(111,axisbg='gray')
		vmin=args.vmin_cross
		vmax=args.vmax_cross
		dir_slice_num=args.dir_slice_num_cross
		if rotated is True:
			direction='yz'
			dist=yvec
		else:
		...
		for n in range(1):
			...
			if use_days_title is True:
				title='Time = '+ time_str + ' days'
			datamap2 =plot_data_field(data1, y1, z1, vmin, vmax, flipped=False, colorbar=False,\
					cmap=cmap,title=title,ylabel='Depth (m)', colorbar_units=args.colorbar_units, return_handle=True)
			#xmin=450.  ;xmax=750.
			...
			text(0.075,1.0,letter_labels[1], ha='right', va='bottom',transform=ax.transAxes,fontsize=20)
			plt.xlabel('y (km)',fontsize=20)
			#ax.set_xticks([])
			
			fig.subplots_adjust(right=0.85)
			cbar_ax = fig.add_axes([0.88,0.12 , 0.025, 0.33])
			cbar=fig.colorbar(datamap2, cax=cbar_ax)
			cbar.set_label(args.colorbar_units, rotation=90,fontsize=20)
			cbar.ax.tick_params(labelsize=20)
