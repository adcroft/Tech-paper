...
def main():
	...
	if plot_horizontal_field is True:
		...
		(data2, time2)=load_and_compress_data(Drift_iceberg_file,field=field,time_slice=time_slice,time_slice_num=-1,rotated=rotated,return_time=True) 
		(data3,time3)=load_and_compress_data(Bond_iceberg_file,field=field,time_slice=time_slice,time_slice_num=-1,rotated=rotated,return_time=True) 
		...
		plt.subplot(2,2,2)
		plot_data_field(data2,x,y,vmin,vmax,flipped,colorbar=True,cmap='jet',title='No Bonds',xlabel='x (km)',ylabel='y (km)', ylim_min=500.,ylim_max=700.)	
		plt.plot(xvec,grounding_line, linewidth=3.0,color='black')
		plt.subplot(2,2,1)
		plot_data_field(data3,x,y,vmin,vmax,flipped,colorbar=True,cmap='jet',title='Bonds',xlabel='x (km)',ylabel='y (km)', ylim_min=500.,ylim_max=700.)	
		plt.plot(xvec,grounding_line, linewidth=3.0,color='black')
	...
	if plot_temperature_cross_section is True:
		...
		plt.subplot(2,2,4)
		plot_data_field(data2, y2, z2, vmin, vmax, flipped=False, colorbar=True, cmap=cmap, xlim_min=500.,xlim_max=700.,xlabel='y (km)',ylabel='z (m)')
		plt.subplot(2,2,3)
		plot_data_field(data3, y3, z3, vmin, vmax, flipped=False, colorbar=True, cmap=cmap, xlim_min=500.,xlim_max=700.,xlabel='y (km)',ylabel='z (m)')
