...
for k=1:2
    ...
    mult2=0.15*H;
    %fill(x0+mult2.*cos(circ_ind),y0+mult2.*sin(circ_ind),'b');
    %plot(x0+(H.*cos(circ_ind)),y0+(H.*sin(circ_ind)),'Color', 'k','linewidth',0.5 )
    %plot([x0 x0],[y0-H y0+H],'Color', 'm','linestyle',':','linewidth',0.5 )
    %colormap('hot')
end
